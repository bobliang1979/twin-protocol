...
import math, json, hashlib, weakref
import numpy as np
...
class StructuredReasoningNode:
    ...
    __slots__ = ('state', 'depth', 'visits', 'total_value', 'children',
                 '__weakref__', '_parent_ref', 'kv_cache_token')
...
class AstraeaProductionEngine_v4_0:
    ...
    Bugfix Release (3 fixes):
    - FIX-1: batch_expand_and_evaluate visits inflation (child.visits = 1, was 0)
    - FIX-2: run_pipeline synthetic visit removed (is_new_node guard)
    - FIX-3: __slots__ missing __weakref__ (weakref creation crashed all children)
    """
    ...
    async def batch_expand_and_evaluate(self, node: StructuredReasoningNode) -> List[StructuredReasoningNode]:
        ...
        for child, score in zip(unique, scores):
            ...
            child.visits = 1  # FIX-1: was 0. Children born evaluated carry their visit count.
    ...
    async def run_pipeline(self, query: str, max_iterations: int = 8,
        ...
        for iteration in range(max_iterations):
            ...
            # FIX-2 removed: phantom is_new_node visit inflation.
            # leaf.visits was set to 1 here, then incremented again by _backpropagate,
            # giving 2 visits for 1 reward and halving avg_value.
