...
const SCENES = [
  { from: "alice", type: "message", text: "Hey Bob, ready to build a web app together?", wait: 0.5 },
  { from: "bob", type: "message", text: "Ready! I'll handle the Node.js backend.", wait: 0.8 },
  { from: "alice", type: "tool_request", tool: "shell.run", params: { command: "echo 'Starting...'" }, wait: 1.0 },
  { from: "bob", type: "tool_result", tool: "shell.run", result: { stdout: "Starting...", exit_code: 0 }, wait: 0.3 },
  { from: "bob", type: "tool_request", tool: "js.eval", params: { code: "2 + 2" }, wait: 0.5 },
  { from: "alice", type: "tool_result", tool: "js.eval", result: { stdout: "4", type: "number" }, wait: 0.3 },
  { from: "alice", type: "message", text: "API contract: POST /api/data -> JSON. Agreed?", wait: 1.2 },
  { from: "bob", type: "message", text: "Agreed! Building endpoint now. This might take a moment...", wait: 2.0 },
  { from: "alice", type: "state_update", state: { phase: "building", task: "data module" }, wait: 0.5 },
  { from: "bob", type: "state_update", state: { phase: "building", task: "API endpoint" }, wait: 1.5 },
  { from: "bob", type: "message", text: "Endpoint ready at http://localhost:8080/api", wait: 1.0 },
  { from: "alice", type: "message", text: "Testing the endpoint... Works! Deploying now.", wait: 1.5 },
  { from: "alice", type: "message", text: "Two agents, one protocol, zero friction. That's Twins.", wait: 1.0 },
  { from: "bob", type: "message", text: "🤝 Deployed and verified. Collaboration complete.", wait: 0.5 },
];
